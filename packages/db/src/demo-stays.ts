 ...
 *
 * Callers: `scripts/refresh-demo-stays.ts` (by hand) and the nightly `demo-refresh` job. The demo
 * went stale silently once already; a tool that depends on being remembered is one that stops.
 */
...
export interface DemoRefreshResult {
  /** Human-readable trace, so the CLI and the job log say exactly the same thing. */
  lines: string[];
...
/**
 * @param apply  false (the default) reports what WOULD change and writes nothing.
 */
export async function refreshDemoStays({ apply = false }: { apply?: boolean } = {}): Promise<DemoRefreshResult> {
  ...
  if (tenants.length === 0) {
    throw new Error("No demo tenants. Refusing to touch anything.");
  }

  log(`${APPLY ? "APPLYING" : "DRY RUN"} — demo tenants: ${tenants.map((t) => t.name).join(", ")}\n`);

  for (const tenant of tenants) {
    const property = tenant.properties[0];
    if (!property) {
      log(`${tenant.name}: no property, skipped\n`);
      continue;
    }
    ...
    if (units.length === 0 || !ratePlan) {
      log(`${tenant.name}: ${units.length} unit rows, ratePlan=${!!ratePlan} — skipped\n`);
      continue;
    }
    ...
    log(`${tenant.name} — ${property.name} (${property.timezone}, today is ${todayIso})`);
    log(
    ...
    log(`  ✓ ${planned.length} stays written for ${tenant.name}`);
    tenantsTouched++;
    staysWritten += planned.length;
  }
