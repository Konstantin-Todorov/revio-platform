/**
 * Calendar-date helpers for a stay — shared, because a second product needed the same calendar.
 *
 * Moved here from `apps/booking/lib/dates.ts` when RevioCRS's availability search got the same
 * two-month range picker the booking engine already had (E2 / §3.3). The rule this platform follows
 * is that a thing moves to a package at the moment a second caller appears, never before.
 *
 * Two rules run through all of it. **"Today" comes from the LOCAL calendar** — someone in UTC+3 at
 * one in the morning must not be told they are trying to book yesterday. **Everything else is
 * anchored to UTC midnight**, because a stay is a run of calendar dates rather than an instant, and
 * doing the arithmetic in local time makes a night vanish or double on a DST boundary.
 */
