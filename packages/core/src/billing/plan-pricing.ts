import type { Entitlements } from "../domain/types.js";

/**
 * The pricing model — what we charge hotels, and the only place it is decided.
 *
...
/**
 * ⚠️ `BilledProductKey`, not `ProductKey` — this codebase has two spellings of a product.
 *
 * `ProductKey` in `products/products.ts` is `"cm" | "crs" | "pms"`: the short key a URL, an email and
 * a `ProductTrial` row use. This one is `"channelManager" | "reservation" | "pms"`, the shape of the
 * `Tenant` entitlement COLUMNS, which is what a price is computed from.
 *
 * They were two types with one name while this module lived in the operator app, and moving it here
 * is what forced the distinction to be written down. Mixing them up is a real bug class: it already
 * produced a mislabelled product name once this week, on the operator client page.
 */
export type BilledProductKey = keyof Entitlements;
...
export const MODULE_MINOR: Record<BilledProductKey, number> = { channelManager: 4900, reservation: 5900, pms: 6900 };
export const MODULE_LABEL: Record<BilledProductKey, string> = { channelManager: "RevioLink", reservation: "RevioCRS", pms: "RevioPMS" };
export const MODULE_SHORT: Record<BilledProductKey, string> = { channelManager: "CM", reservation: "CRS", pms: "PMS" };
...
export function entitledKeys(ent: Entitlements): BilledProductKey[] {
  return PRODUCT_KEYS.filter((k) => ent[k]);
}

export interface PriceBreakdown {
  platformMinor: number;
  modules: { key: BilledProductKey; label: string; minor: number }[];
...
export interface RevenueAttribution {
  byProduct: Record<BilledProductKey, number>;
...
export function attributeRevenue(plan: string, ent: Entitlements): RevenueAttribution {
  ...
  const keys = entitledKeys(ent);
  const byProduct: Record<BilledProductKey, number> = { channelManager: 0, reservation: 0, pms: 0 };
...
/**
 * The products a tenant is actually BILLED for — their entitlements, minus anything on a free trial.
 *
 * ## ⚠️ The defect this exists to close
 *
 * A trial is an entitlement flag. `Tenant.hasPms` is `true` during a RevioPMS trial, because that is
 * exactly how the hotel gets access. `generateInvoices` computed the month's price straight from
 * those flags, so a hotel on a free trial was invoiced for it — the product page, the trial email,
 * the self-serve trial screen and the banner at the top of their own product all promise *"nothing
 * is charged"*, and the invoice loop had never heard of any of them.
 *
 * It is worse than one line, too: the **bundle discount is priced by the NUMBER of modules**, so a
 * third product arriving on trial re-prices the two they really do pay for. The error is not
 * confined to the thing that was free.
 *
 * Caught on 2026-09-11 while building the hotel's own billing screen — which would have shown the
 * customer the wrong figure with our name on it. No real client had been invoiced yet, which is the
 * only reason this was a defect rather than a refund.
 *
 * ## The rule, and why it is this one
 *
 * A product on a trial that is **still running when the invoice is cut** is not billed for that
 * period. Converting a trial ends it (`endedAt` is set the moment somebody presses "They kept it"),
 * so a kept product is billed from the very next invoice with no further bookkeeping. Nothing is
 * pro-rated, because nothing else in this model is: an invoice is a calendar month.
 *
 * It deliberately errs toward the customer. The alternative — bill unless a trial covered the whole
 * period — would charge somebody for a month they spent evaluating, which is the one bill guaranteed
 * to end an evaluation badly.
 */
export function billableEntitlements(
  ent: Entitlements,
  /** Products with a RUNNING trial, in the short `"cm" | "crs" | "pms"` spelling a trial row uses. */
  onTrial: readonly string[],
): Entitlements {
  const COLUMN: Record<string, BilledProductKey> = {
    cm: "channelManager",
    crs: "reservation",
    pms: "pms",
  };
  const out: Entitlements = { ...ent };
  for (const key of onTrial) {
    const column = COLUMN[key];
    // An unknown product key is ignored rather than guessed at. Silently zeroing a column because a
    // string did not match would be the same class of bug in the opposite direction.
    if (column) out[column] = false;
  }
  return out;
}
