...
| **RevioLink Sync Center · Error Center · Audit** ✅ already one screen with tabs since V2 (this row was wrong); now on the shared tabs | three screens | One **Activity** area, tabs: **Syncs · Errors · Changes** | Three views of "what happened", chosen by the question, not the table |
| **RevioLink Bulk update + Restrictions** ✅ one screen since V2; tabs added 2026-09-25, RevioCRS Bulk the same | two screens | Tabs on one screen: **Change prices & availability · Your rules** | The rules are the standing version of a bulk change |
