...
### ⚠️ NEEDS A PERSON — the invoice job is deployed and has never run

`operator.reviosoft.app/api/health/jobs` on 2026-09-13 at 16:48 local:

```
arrivals-digest   86s ok      auto-assign      185s ok
channex-pull      86s ok      auto-close-day   185s ok
hold-expiry      180s ok      trial-sweep      185s ok
pickup-snapshot  180s ok      waitlist-sweep   185s ok
support-inbox    182s ok      invoice-run       — never
```

Every other job the cron runner drives ran within the last three minutes, so **the runner is alive
and the operator origin resolves**. `invoice-run` has never run at all. It is declared in `JOB`,
routed at `/api/jobs/invoices`, listed in `scripts/run-jobs.mjs`, `jobs-lint` passes 10 of 10, and
the commit carrying all three is on `production`.

The only explanation left is that **the Railway cron service is still executing an older copy of
`scripts/run-jobs.mjs`** — it needs a redeploy. Nobody can do that from here; Railway CLI auth is
not available in this session.

⚠️ This is the third time this exact shape has appeared (root `CLAUDE.md`: `trial-sweep` spent its
whole life POSTing into a login page; the waitlist sweep shipped and never ran). The dead-man's
switch caught it within ten minutes this time, which is what it is for. **Until the cron service is
redeployed, invoices are still only generated by pressing the button in the console.**
