 ...
 * is where the data is: `getNotificationFeed` refuses anyone without an operator identity.
 */
...
export async function markNotificationRead(key: string): Promise<void> {
  ...
  await prisma.operatorUser.updateMany({
  ...
  const row = await prisma.operatorUser.findUnique({
