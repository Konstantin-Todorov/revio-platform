...
export async function GET(req: NextRequest) {
   ...
   */
  const gate = await checkLoginAllowed("crs", `handoff:${origin.ip ?? "unknown"}`);
  if (!gate.allowed) {
    login.searchParams.set("error", "Too many attempts. Wait a moment and sign in normally.");
    return NextResponse.redirect(login);
  }

  const result = await consumeHandoff(token, "crs");
  if (!result.ok) {
    await recordLoginFailure("crs", `handoff:${origin.ip ?? "unknown"}`);
  ...
  if (decision.kind !== "open") {
    ...
    login.searchParams.set("error", "RevioCRS is not open on this account right now.");
    return NextResponse.redirect(login);
  }
  ...
  await recordLoginSuccess("crs", `handoff:${origin.ip ?? "unknown"}`);
