...
export default async function SettingsUsersPage() {
  ...
  const U = (await i18n()).t(settingsDict).users;
  const GROUP_LABEL: Record<string, string> = U.groups;
  ...
  return (
    <>
      <Card>
        <CardHeader title={U.title} subtitle={U.subtitle} />
          ...
          <table className="w-full text-[12.5px]">
            <thead>
              <tr className="border-b border-surface-border text-left text-[11px] font-semibold uppercase tracking-wide text-ink-400">
                <th className="px-4 py-2.5">{U.role}</th>
              ...
              {roles.map((role) => (
                <tr key={role.id} className="group border-b border-surface-border/60 last:border-0">
                  <td className="px-4 py-2">
                    <span className="font-semibold text-ink-900">{role.builtin ? U.builtinNames[role.name] ?? role.name : role.name}</span>
                    {role.builtin && <span className="ml-1.5 rounded bg-surface-sunken px-1 text-[9.5px] font-bold uppercase text-ink-400">{U.builtin}</span>}
                  </td>
                    ...
                    return (
                      <td key={g} className="px-2 py-2 text-center">
                        <span className={`inline-block min-w-[38px] rounded px-1.5 py-0.5 text-[10.5px] font-bold uppercase ${
                          ...
                          {U.levels[level as keyof typeof U.levels] ?? level}
                        </span>
                      </td>
                    );
                  })}
                  <td className="px-2 py-2 text-right">
                    {!role.builtin && (
                      <span className="opacity-0 transition-opacity group-hover:opacity-100">
                        <DeleteButton action={deletePermissionRole} id={role.id} label={role.name} note={U.deleteNote} />
                      </span>
                    )}
                  </td>
                </tr>
        ...
        <details className="border-t border-surface-border/60">
          ...
          <form action={savePermissionRole} className="grid grid-cols-2 items-end gap-3 p-4 lg:grid-cols-5">
            <div className="col-span-2 lg:col-span-1">
              <label className={labelCls}>{U.roleName}</label>
              <input name="name" required placeholder={U.rolePlaceholder} className={inputCls} />
            </div>
            {PERMISSION_GROUPS.map((g) => (
              <div key={g}>
                <label className={labelCls}>{GROUP_LABEL[g]}</label>
                <select name={`level_${g}`} defaultValue="none" className={inputCls}>
                  <option value="none">{cap(U.levels.none)}</option>
                  <option value="view">{cap(U.levels.view)}</option>
                  <option value="edit">{cap(U.levels.edit)}</option>
                </select>
              </div>
            ))}
            <button className="h-[34px] rounded-md bg-brand-800 px-3.5 text-[12.5px] font-semibold text-white transition-colors hover:bg-brand-700">{U.createRole}</button>
          </form>
        </details>
      </Card>
      ...
      <Card>
        <CardHeader
          title={U.staffTitle(users.length)}
          subtitle={U.staffSub}
        />
        <StaffManagement
