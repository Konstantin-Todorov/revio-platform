...
export async function login(_prev: LoginResult | null, fd: FormData): Promise<LoginResult> {
  ...
  const gate = await checkLoginAllowed("crs", email);
  if (!gate.allowed) {
    await recordAuthEvent({ scope: "crs", type: AUTH_EVENT.signInBlocked, email, ...origin, detail: "rate limited" });
    return { error: gate.message };
  }

  const user = await prisma.user.findUnique({ where: { email }, include: { tenant: true } });
  if (!user || !user.passwordHash || !(await verifyPassword(password, user.passwordHash))) {
    await recordLoginFailure("crs", email);
    ...
    await recordAuthEvent({
      scope: "crs", type: AUTH_EVENT.signInFailed, email,
      userId: user?.id ?? null, tenantId: user?.tenantId ?? null, ...origin,
    });
    return { error: "Invalid email or password." };
  }
  await recordLoginSuccess("crs", email);
   ...
   * screen is for — were met at the door with "RevioCRS isn't enabled for this hotel", which is the
  ...
  await recordAuthEvent({
    scope: "crs", type: AUTH_EVENT.signIn,
...
export async function verifyTwoFactor(_prev: LoginResult | null, fd: FormData): Promise<LoginResult> {
  ...
  const gate = await checkLoginAllowed("crs", `2fa:${pending.userId}`);
  if (!gate.allowed) return { error: gate.message };

  const origin = requestOrigin(await headers());
  const result = await verifyUserSecondFactor(pending.userId, code);
  if (!result.ok) {
    await recordLoginFailure("crs", `2fa:${pending.userId}`);
    await recordAuthEvent({
      scope: "crs", type: AUTH_EVENT.twoFactorFailed,
      userId: pending.userId, ...origin, detail: result.error,
    });
    return { error: result.error };
  }
  await recordLoginSuccess("crs", `2fa:${pending.userId}`);
