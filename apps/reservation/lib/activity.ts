...
import { addDays } from "@revio/core";
import { utcDay } from "./mutation-helpers";
import { todayInTz } from "./data";
 ...
 *
 * The same AuditEntry stream the PMS reads, deliberately: a rate edited here and a folio voided
 * there are one hotel's history, and a manager asking "who changed this" should not have to know
 * which product wrote the row. RevioCRS is sold on its own, so a client who never bought the PMS
 * would otherwise have no way to see any of it.
 */
export async function getActivity(opts: {
  ...
  const from = ISO.test(opts.from ?? "") ? opts.from! : addDays(to, -6);

  const where = {
    propertyId: property.id,
    createdAt: { gte: utcDay(from), lt: utcDay(addDays(to, 1)) },
