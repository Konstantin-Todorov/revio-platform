...
/** Preview only — see apps/pms/components/shell/sidebar-fit.test.tsx for what is being measured. */
it("writes a preview when asked", () => {
  const file = process.env.SIDEBAR_PREVIEW;
  if (!file) return;
  const css = process.env.SIDEBAR_CSS ?? "";
  const html = renderToStaticMarkup(React.createElement(Sidebar, { footer: "Hotel Sofia Group" }));
  writeFileSync(
    file,
