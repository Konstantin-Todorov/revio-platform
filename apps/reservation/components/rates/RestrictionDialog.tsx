...
const SOURCE_CATEGORIES: [string, string][] = [
  ["direct", "Direct"], ["ota", "OTA"], ["gds", "GDS"],
  ["call_center", "Call Center"], ["corporate", "Corporate"], ["travel_agent", "Travel Agent"],
];
    ...
    <>
      ...
      <Modal open={open} onClose={() => setOpen(false)} title={isEdit ? `Edit ${rule!.name}` : "Add Restriction Rule"}>
        <form action={formAction} className="space-y-3.5">
          ...
          <div>
            <span className="mb-1.5 block text-[12px] font-semibold text-ink-700">Booking sources <span className="font-normal text-ink-400">(none checked = every source)</span></span>
            <div className="flex flex-wrap gap-1.5">
              {SOURCE_CATEGORIES.map(([v, l]) => (
