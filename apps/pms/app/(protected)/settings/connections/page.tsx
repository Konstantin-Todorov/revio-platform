...
export default async function ConnectionsSettingsPage() {
  ...
  const t = tr(settings).connections;

  return (
    <Card>
      <CardHeader
        title={t.title}
        action={<span className="text-[11.5px] text-ink-400">{t.managed}</span>}
      />
      {channels.length === 0 ? (
          ...
          {channels.map((c) => (
            <li key={c.id} className="flex flex-wrap items-center justify-between gap-3 px-4 py-2.5">
              <div className="flex items-center gap-2.5">
                ...
                <span className="text-[11px] text-ink-400">{t.modes[c.connectivityMode] ?? connectivityModeLabel(c.connectivityMode)}</span>
              </div>
              <StatusPill tone={(c.status === "connected" ? "success" : "neutral") as Tone}>{t.statuses[c.status] ?? c.status}</StatusPill>
            </li>
          ))}
        </ul>
      )}
