...
export default async function GuestsPage() {
  ...
  return (
    <div>
      ...
      <Card>
        <CardHeader title={t.withStay(rows.length)} action={<span className="flex items-center gap-1 text-[12px] font-semibold text-ink-400"><Users className="h-3.5 w-3.5" />{rows.length}</span>} />
        {rows.length === 0 ? (
          <div className="px-4 py-8 text-center text-[13px] text-ink-400">{t.none}</div>
        ) : (
          <GuestsTable
