...
export async function GET(req: NextRequest) {
   ...
   */
  const gate = await checkLoginAllowed("pms", `handoff:${origin.ip ?? "unknown"}`);
  if (!gate.allowed) {
    login.searchParams.set("error", "Too many attempts. Wait a moment and sign in normally.");
    return NextResponse.redirect(login);
  }

  const result = await consumeHandoff(token, "pms");
  if (!result.ok) {
    await recordLoginFailure("pms", `handoff:${origin.ip ?? "unknown"}`);
  ...
  if (decision.kind !== "open") {
    ...
    login.searchParams.set("error", "RevioPMS is not open on this account right now.");
    return NextResponse.redirect(login);
  }
  ...
  await recordLoginSuccess("pms", `handoff:${origin.ip ?? "unknown"}`);
   ...
   */
  const ttl = 12 * 60 * 60;
  const res = NextResponse.redirect(new URL(roleHome(user.role), req.nextUrl.origin));
