...
export async function submitSupportRequest(_prev: GetHelpResult, fd: FormData): Promise<GetHelpResult> {
  ...
  if (!session) {
    return { ok: false, error: (await flashSay()).support.expired };
  }
  ...
  try {
    await sendEmail({
      ...
      subject: `[${k.key.toUpperCase()}] ${result.reference} · ${session.tenantName} · RevioPMS`,
...
/**
 * Answer us back, from inside RevioPMS.
 *
 ...
 *
 * The perimeter is here, as it is for every write: this action resolves RevioPMS's own session and
...
export async function replyToSupport(
  ...
  const session = await getSession();
  if (!session) return { ok: false, error: (await flashSay()).support.expired };

  const requestId = String(fd.get("requestId") ?? "");
  const body = String(fd.get("body") ?? "");
  if (!requestId) return { ok: false, error: (await flashSay()).support.unknownRequest };
  ...
  const mail = {
    ...
    product: "RevioPMS",
    blocks: [
      { list: [`Hotel — ${session.tenantName}`, `From — ${session.userName} (${session.role})`, `Product — RevioPMS`] },
      { p: body },
  ...
  try {
    await sendEmail({
      to: [process.env.SUPPORT_INBOX?.trim() || "office@reviosoft.app"],
      subject: `Re: ${reference} · ${session.tenantName} · RevioPMS`,
      text: renderSystemEmailText(mail),
      html: renderSystemEmail(mail),
    });
  } catch {
