...
export async function submitSupportRequest(_prev: GetHelpResult, fd: FormData): Promise<GetHelpResult> {
  ...
  try {
    await sendEmail({
      ...
      subject: `[${k.key.toUpperCase()}] ${result.reference} · ${session.tenantName} · RevioLink`,
...
/**
 * Answer us back, from inside RevioLink.
 *
 ...
 *
 * The perimeter is here, as it is for every write: this action resolves RevioLink's own session and
...
export async function replyToSupport(
  ...
  const mail = {
    ...
    product: "RevioLink",
    blocks: [
      { list: [`Hotel — ${session.tenantName}`, `From — ${session.userName} (${session.role})`, `Product — RevioLink`] },
      { p: body },
  ...
  try {
    await sendEmail({
      to: [process.env.SUPPORT_INBOX?.trim() || "office@reviosoft.app"],
      subject: `Re: ${reference} · ${session.tenantName} · RevioLink`,
      text: renderSystemEmailText(mail),
      html: renderSystemEmail(mail),
    });
  } catch {
