 ...
 *
 * `(protected)/layout.tsx` renders `ProductLocked` when RevioLink is off — but Next runs a server
...
describe("the RevioLink entitlement gate", () => {
  ...
  it("⚠️ refuses the write when RevioLink is switched off", async () => {
    state.value = { ...session, entitlements: { ...session.entitlements, channelManager: false } };
    const r = await guard("manageRates");
