...
/**
 * Request-scoped, tenant-isolated Prisma access.
 *
 ...
 *
 * Files keep their existing `prisma.foo.bar(...)` call sites unchanged — only the import swaps from
 * `@revio/db` to this module. Identity resolution that runs before a tenant exists (session, login)
 * must NOT use this; it uses `forSystem()` directly.
 */
